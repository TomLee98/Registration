classdef regrcf < handle
    ...
    properties(SetAccess=immutable, Hidden)
        sfolder         % folder with rcf files, as "virtual memory" part
        volopt
        regopt
        distrib
    end
    ...
    methods
        ...
        function [rcfpool, flist] = readall(this, owner)
            arguments
                this 
                owner   (1,:)   string    = string([])
            end
            ...
            if isfolder(this.sfolder)
                ...
                for n = 1:numel(xml_files)
                    ...
                    if nargout == 2
                        % replace with current uid
                        if isempty(owner), owner = this.data.user_id; end
                        if isequal(s.user_id, owner)
                            flist = [flist; fullfile(this.sfolder, xml_files{n})]; %#ok<AGROW>
                        end
                    end
                end
            else
        ...
        function tf = current_await_with_rcf(this)
            % This function determines current user status is await, 
        ...
        function delete(this)
            % remove all rcf file generated by current user
            % to avoid some broken caused block files dead lock
            [~, flist] = this.readall();
